...
- `docker build -t <image-name> <build-context-path>` Build an image. Build context path is where the Dockerfile is located (e.g. `.` if in current directory) [reference](https://docs.docker.com/engine/reference/commandline/build/).
- `docker images` List available images [reference](https://docs.docker.com/engine/reference/commandline/images/).
- `docker rmi <image-name>` Remove image [reference](https://docs.docker.com/engine/reference/commandline/rmi/).
...
- `docker create --name <container-name> -m "300M" --memory-swap "1G" <image-name>` Create a container [reference](https://docs.docker.com/engine/reference/commandline/create/).
- `docker start ... <container-name>` Start a created container [reference](https://docs.docker.com/engine/reference/commandline/start/).
- `docker run -d --name <container-name> ... <image-name>` Shorthand for create + start [reference](https://docs.docker.com/engine/reference/commandline/run/). Without `-d` connects to container and shows console output. Ctrl-C disconnects terminal but leaves container running.
- `docker ps --all` Show existing containers [reference](https://docs.docker.com/engine/reference/commandline/ps/).
...
Stopping an removing the container does not remove volumes created with the docker tools.
To reset a volume on a container, it is required to remove the volume with docker tools (instead of manual volume dorectory removal).
...
- `docker inspect <container-name>` Show container information [reference](https://docs.docker.com/engine/reference/commandline/inspect/).
- `docker attach <container-name>` Attach terminal to container [reference](https://docs.docker.com/engine/reference/commandline/attach/). NOTE detaching from the terminal does not work as described in the docs. For example, if attached from remote terminal with docker toolbox tools, Ctrl-C detaches the terminal instead of killing the process (Ctrl-P + Ctrl-Q does nothing). If ssh'ed on remote host and attached from there, the terminal cannot be detached or the process killed with any key combination (must kill the terminal). TODO container must be started with `-i` flag for this to work predictably? `-t` required as well? [see here](https://docs.docker.com/engine/quickstart/#running-an-interactive-shell)
- `docker logs --tail=<no-of-lines> <vm-name>` show console output. Add `--follow` flag to keep following the log (use Ctrl-C to exit) [reference](https://docs.docker.com/engine/reference/commandline/logs/).
- `docker stop -t <timeout-seconds> <container-name>` Terminate container with SIGTERM and SIGKILL after timeout-seconds (default: 10 seconds) [reference](https://docs.docker.com/engine/reference/commandline/kill/).
- `docker kill <container-name>` Terminate container with SIGKILL [reference](https://docs.docker.com/engine/reference/commandline/kill/).
- `docker exec -it <container-name> bash` Open a terminal inside the container [reference](https://docs.docker.com/engine/reference/commandline/exec/).
