...
Without `-d` connects to container and shows console output.
Ctrl-C disconnects terminal but leaves container running.

`docker ps --all`

`docker logs --follow <VM name>` show console output (`--follow` flag tails output).
